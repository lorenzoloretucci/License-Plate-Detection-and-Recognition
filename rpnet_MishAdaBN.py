...
def main():
    ...
    ap.add_argument("-w", "--writeFile", default='fh02_mada.out',
                    help="file for output")
    args = vars(ap.parse_args())

    wR2Path = 'C:/Users/marco/wR2_MishAdaBN_try3/wR2_MishAdaBN_try3.pth6'
    ...
    def eval(model, test_dirs):
        ...
        for i, (XI, labels, ims) in enumerate(testloader):
            ...
            #   compare YI, outputY
            try:
                if isEqual(labelPred, YI[0]) >= 4:
                    correct += 1
                    #correct += isEqual(labelPred, YI[0])/7
                else:
                    pass
            except:
                error += 1
        print(correct, count, error)
    ...
    def train_model(model, criterion, optimizer, num_epochs=25):
        ...
        for epoch in range(epoch_start, num_epochs):
            ...
            print ('%s %s %s\n' % (epoch, sum(lossAver) / (len(lossAver) + 1e-6), time()-start))
            model.eval()
...
if __name__ == '__main__':
    ...
    print('loop')
    main()
## python rpnet\rpnet.py -i rpnet\train_img -b 4 -se 0 -f rpnet\model -t rpnet\test_AML
## python rpnet\rpnet_MishAdaBN.py -i rpnet\immagine -n 10 -b 4 -se 0 -f rpnet\model -t rpnet\validation_immagine